...
# تم تغيير مرآة التوزيع إلى مرآة Ubuntu بدلاً من مرآة ديبيان
...
while [[ $# -gt 0 ]]; do
	case $1 in
		...
		--nochrony)
			...
			;;
		--debianmirror)
			# لم يعد يُستخدم هذا الخيار، لأننا نعتمد على مرآة Ubuntu الآن
...
setup_repositories() {
	...
	# تم تعديل سطور المستودعات لاستخدام codename الخاص بتوزيعة Ubuntu
	if [ "$testrepo" ] ; then
		add-apt-repository -y -S "deb [ arch=amd64 ] http://deb.freepbx.org/freepbx17-dev $(lsb_release -sc) main" >> "$log"
		add-apt-repository -y -S "deb [ arch=amd64 ] http://deb.freepbx.org/freepbx17-dev $(lsb_release -sc) main" >> "$log"
	else
		add-apt-repository -y -S "deb [ arch=amd64 ] http://deb.freepbx.org/freepbx17-prod $(lsb_release -sc) main" >> "$log"
		add-apt-repository -y -S "deb [ arch=amd64 ] http://deb.freepbx.org/freepbx17-prod $(lsb_release -sc) main" >> "$log"
	fi

	# تعديل المستودع الخاص بالحزم غير المفتوحة بحيث يستخدم مكونات Ubuntu القياسية
	if [ ! "$noaac" ] ; then
		add-apt-repository -y -S "deb $UBUNTU_MIRROR $(lsb_release -sc) main restricted universe multiverse" >> "$log"
	fi
...
# Adding iptables and postfix inputs so "iptables-persistent" and postfix will not ask for input
